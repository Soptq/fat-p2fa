...
const P2FA: Page = () => {
  ...
  const onVerify = async () => {
    if (!certificateData || !contract) return
    const {output} = await contract.query.verifyToken(certificateData as any, {}, p2faVerifyToken.join(''))
  ...
  return (
      <>
          ...
          <ParagraphMedium>
              This application is possible to serves as <strong>a second layer for wallet protections</strong>. Nowadays, the loss of private keys or mnemonic phrases generally predicts the loss of wallets. However, if one enables 2FA on chain, even the private keys are lost, funds can be still safu.
          </ParagraphMedium>
          {
              contract ? (
                  certificateData ? (
                      <>
                          <ParagraphMedium>2FA enabled for your account: {enabled2fa}</ParagraphMedium>
                          <HeadingMedium as="h1">1. Initialize 2FA</HeadingMedium>
                          <ParagraphMedium>Before using 2FA to protect your on-chain data, you firstly need to initialize a secret key for your 2FA verification process. Thanks to Phala Fat Contract, the secret generating process can be achieved in the contract safely.</ParagraphMedium>
                          <Button disabled={!certificateData} onClick={onInit}>
                              {initialized ? "Re-initialize 2FA (Reset Secret)" : "Initialize 2FA" }
                          </Button>
                          { bindingURL && (
                              <>
                          ...
                          <ParagraphMedium>The 2FA will be enabled after successfully verifying the token. This would allow the contract to make sure the token generated by your authenticator works well with it.</ParagraphMedium>
                          ...
                          <ParagraphMedium>Finally, after initializing and binding, your 2FA is successfully enabled. Now any third party application can use this 2FA to verify your identity. Try it yourself~</ParagraphMedium>
                          <div className={css({display: 'flex'})}>
                              <PinCode
